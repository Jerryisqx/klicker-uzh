...
export async function UploadFileAPI(
  ...
  try {
    console.log('Environment FILE_UPLOAD_URL:', process.env.FILE_UPLOAD_URL)
    ...
    console.error('Error Uploading Files calling API', error)
  }
}
...
//Get total number of questions generated by a same user
export async function getGeneratedQuestionsCount(
  ctx: ContextWithUser,
  ownerId: string
) {
  const total = await ctx.prisma.elementGenerated.count({
    where: {
      ownerId: ownerId,
    },
  })
